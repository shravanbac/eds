...
async function getAemPlugins(owner, repo) {
  const origin = ref === 'local' ? 'http://localhost:3001' : `https://${ref}--${repo}--${owner}.aem.live`;
  const confUrl = ref === 'local' ? 'http://localhost:3001/tools/sidekick/config.json' : `https://admin.hlx.page/sidekick/${owner}/${repo}/${ref}/config.json`;
...
function calculateSources(org, repo, sheetPath) {
  return sheetPath.split(',').map((path) => {
    ...
    if (ref === 'local') return `http://localhost:3001${path}`;
